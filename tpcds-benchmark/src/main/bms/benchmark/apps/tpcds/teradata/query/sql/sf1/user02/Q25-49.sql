-- start query 13 in stream 0 using template q49.tpl and seed 1200888605
...
sel
 ...
 from (
        ...
        from
        (       select 
                cs1.cs_item_sk as item
                ,(cast(sum(coalesce(cr.cr_return_quantity,0)) as dec(15,4))/
                cast(sum(coalesce(cs1.cs_quantity,0)) as dec(15,4) )) as return_ratio
                ,(cast(sum(coalesce(cr.cr_return_amount,0)) as dec(15,4))/
                cast(sum(coalesce(cs1.cs_net_paid,0)) as dec(15,4) )) as currency_ratio
                from 
                catalog_sales cs1 left outer join catalog_returns cr
                        on (cs1.cs_order_number = cr.cr_order_number and 
                        cs1.cs_item_sk = cr.cr_item_sk)
                ,date_dim
                where 
                        cr.cr_return_amount > 10000 
                        and cs1.cs_net_profit > 1
                         and cs1.cs_net_paid > 0
                         and cs1.cs_quantity > 0
                         and cs_sold_date_sk = d_date_sk
                         and d_year = 1998
                         and d_moy = 11
                 group by cs1.cs_item_sk
        ) in_cat
 ) catalog
 where 
 (
